...
//		result.stream().forEach(ac -> System.out.println(ac.getId() + " " + ac.getContent()));
		...
		//获取01开头的List<AC>
		Predicate<String> startsWith01 = (n) -> n.startsWith("01");
		List<AC> result01 = list.stream().
				map(ac -> //对content进行处理
					new AC(ac.getId(), Arrays.asList(ac.getContent().split(",")).	//拆分原始content内容
							stream().filter(startsWith01).	//筛选01开头的数据
							collect(Collectors.joining(".")))	//整合数据
				).	
				filter(ac -> ac.getContent().length() > 0).	//根据content进行过滤，删掉content为空的记录
				collect(Collectors.toList());
		result01.stream().forEach(ac -> System.out.println(ac.getId() + " " + ac.getContent()));
		
		//获取02开头的List<AC>
//		Predicate<String> startsWith02 = (n) -> n.startsWith("02");
//		List<AC> result02 = list.stream().
//				map(ac -> new AC(ac.getId(), Arrays.asList(ac.getContent().split(",")).stream().filter(startsWith02).collect(Collectors.joining(".")))).
//				filter(ac -> ac.getContent().length() > 0).
//				collect(Collectors.toList());
